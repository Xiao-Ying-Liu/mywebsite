...
```
3. Go back to Overleaf, paste the text in example.bib, change the **key** of the cited article if you need.
4. In your *main.tex*, before `\begin{document}`, `\usepackage{natbib}`
5. At the end of the document, include
```s
\bibliographystyle{jpe}  % the natbib style you use
\bibliography{example}   % include the bib file you created
```
6. In the body of your document, you could use `\cite{key}`, `\citep{key}`, etc. to cite the paper now.
